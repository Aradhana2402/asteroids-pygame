...
class AsteroidField(pygame.sprite.Sprite):
    # Possible spawn edges (direction vector, spawn position function)
    edges = [
        [
            pygame.Vector2(1, 0),
    ...
    def update(self, dt):
        ...
        if self.spawn_timer > ASTEROID_SPAWN_RATE:
            ...
            # Choose random edge to spawn asteroid
            edge = random.choice(self.edges)
            speed = random.randint(40, 100)

            velocity = edge[0] * speed
            velocity = velocity.rotate(random.randint(-30, 30))  # add randomness
            position = edge[1](random.uniform(0, 1))

            # Different asteroid sizes (kind = 1, 2, ..., ASTEROID_KINDS)
